...
export function usePDFOperations() {
  ...
  const { currentDocument, updateDocument, setCurrentDocument } = usePDFStore();
  const { addHistoryState } = useHistoryStore();

  const createPDFFromArrayBuffer = useCallback(async (arrayBuffer: ArrayBuffer): Promise<PDFDocument> => {
    return await PDFDocument.load(arrayBuffer);
  }, []);

  const performPageOperation = useCallback(async (operation: PageOperation) => {
    ...
    try {
      const pdfDoc = await createPDFFromArrayBuffer(await currentDocument.file.arrayBuffer());
      
      switch (operation.type) {
        case 'add':
          const newPage = pdfDoc.addPage();
          if (operation.parameters?.width && operation.parameters?.height) {
            newPage.setSize(operation.parameters.width, operation.parameters.height);
          }
          setProgress(50);
          break;
        ...
        case 'delete':
          operation.pageIds.forEach((pageId, index) => {
            const pageIndex = parseInt(pageId.replace('page_', '')) - 1 - index;
        ...
        case 'move':
          if (operation.targetIndex !== undefined) {
            const pageIndex = parseInt(operation.pageIds[0].replace('page_', '')) - 1;
      ...
      const newFile = new File([pdfBytes], currentDocument.name, { type: 'application/pdf' });
  ...
  const splitDocument = useCallback(async (options: SplitOptions): Promise<Blob[]> => {
    if (!currentDocument) throw new Error('No document loaded');
  ...
  const mergeDocuments = useCallback(async (operation: MergeOperation): Promise<Blob> => {
    setIsProcessing(true);
    setProgress(0);
    ...
    try {
      ...
      for (let i = 0; i < totalFiles; i++) {
        ...
        const pageIndices = Array.from({ length: doc.getPageCount() }, (_, idx) => idx);
  ...
  const extractPages = useCallback(async (range: ExtractRange): Promise<Blob> => {
    if (!currentDocument) throw new Error('No document loaded');
  ...
  const addWatermark = useCallback(async (watermark: Watermark) => {
    if (!currentDocument) throw new Error('No document loaded');
    ...
    try {
      ...
      pages.forEach((page, index) => {
        if (watermark.pages === 'all' || 
            (Array.isArray(watermark.pages) && watermark.pages.includes(index + 1))) {
          ...
          if (watermark.type === 'text') {
            ...
            if (watermark.position.alignment.includes('right')) x = width - 50;
            ...
            page.drawText(watermark.content, {
              ...
              rotate: { type: watermark.style.rotation, angle: watermark.style.rotation },
            });
          }
        }
  ...
  const addHeaderFooter = useCallback(async (headerFooter: HeaderFooter) => {
    if (!currentDocument) throw new Error('No document loaded');
    ...
    try {
      ...
      pages.forEach((page, index) => {
        if (headerFooter.pages === 'all' || 
            (headerFooter.pages === 'except-first' && index > 0) ||
            (Array.isArray(headerFooter.pages) && headerFooter.pages.includes(index + 1))) {
          ...
          const y = headerFooter.type === 'header' 
  ...
  const applySecurity = useCallback(async (security: SecuritySettings) => {
    if (!currentDocument) throw new Error('No document loaded');
    ...
    try {
      ...
      if (security.permissions.copying) {
        permissions.push('copy');
      }
      if (security.permissions.editing !== 'none') {
      ...
      updateDocument(currentDocument.id, {
        file: newFile,
        isEncrypted: security.encryption.enabled,
        updatedAt: new Date(),
      });
  ...
  const exportDocument = useCallback(async (options: ExportOptions): Promise<Blob> => {
    if (!currentDocument) throw new Error('No document loaded');
    ...
    try {
      ...
      if (options.format.type === 'pdf') {
        // Apply export options for PDF
        const saveOptions: any = {};
        
        if (options.security) {
          saveOptions.userPassword = options.security.encryption.userPassword;
          saveOptions.ownerPassword = options.security.encryption.ownerPassword;
        }
        ...
        throw new Error(`Export format ${options.format.type} not yet implemented`);
      }
